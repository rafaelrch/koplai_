-- Script Final para Inserir Agentes
-- Copie e cole este script no SQL Editor do Supabase

-- 1. Desabilitar RLS temporariamente
ALTER TABLE agents DISABLE ROW LEVEL SECURITY;

-- 2. Limpar tabela (opcional)
DELETE FROM agents;

-- 3. Inserir os agentes
...
-- 5. Criar política que permite leitura pública
DROP POLICY IF EXISTS "Allow public read access" ON agents;
CREATE POLICY "Allow public read access" ON agents
  FOR SELECT USING (true);

-- 6. Verificar se os agentes foram inseridos
SELECT COUNT(*) as total_agents FROM agents;
SELECT name, tags FROM agents ORDER BY created_at DESC; 